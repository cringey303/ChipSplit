...
export default function SessionList({ title, sessions, defaultOpen = true }: { title: string; sessions: Session[]; defaultOpen?: boolean; }) {
  ...
  return (
    <div className="w-full rounded-md border border-outline">
      <button
      ...
      >
        ...
        <div className="text-sm text-zinc-500 hover:text-zinc-400">{open ? "Hide" : "Show"}</div>
      </button>
      ...
      {open && (
        <div className="flex flex-col gap-2 px-4 pb-3">
          {sessions.length === 0 ? (
            ...
            sessions.map((s) => (
              <div key={s.id} className="rounded-md border border-outline bg-black px-3 py-2 text-sm">
